...
class SpriteLoader:
    ...
    def load_character_sprites(character_name, base_path=None):
        ...
        if base_path is None:
            base_path = graphics_path("characters")
    ...
    def load_enemy_sprites(enemy_name, base_path=None):
        ...
        """
        if base_path is None:
            base_path = graphics_path("enemies")
    ...
    @staticmethod
    def _resolve_character_dir(base_path, name):
        """Match display names like 'The Engineer' to folders like Engineer/ or AverageJoe/."""
        name_lower = name.lower().strip()
        if name_lower.startswith("the "):
            name_lower = name_lower[4:].strip()
        compact = name_lower.replace(" ", "").replace("_", "")
        candidates = [
            name_lower,
        ...
        if os.path.isdir(base_path):
            for entry in os.listdir(base_path):
                e = entry.lower().replace("_", "")
                if e == compact:
                    return os.path.join(base_path, entry)
        return os.path.join(base_path, name.lower())

    @staticmethod
    def _has_direction_folders(character_dir):
        for d in ("down", "up", "left", "right"):
            p = os.path.join(character_dir, d)
            if os.path.isdir(p):
                return True
        return False
    ...
    def _load_sprites(name, base_path, is_character=True):
        ...
        if is_character:
            character_dir = SpriteLoader._resolve_character_dir(base_path, name)
        else:
        ...
        if os.path.exists(character_dir) and os.path.isdir(character_dir):
            if is_character and not SpriteLoader._has_direction_folders(character_dir):
                pngs = [f for f in os.listdir(character_dir) if f.lower().endswith(".png")]
                pngs.sort()
                if pngs:
                    static_path = os.path.join(character_dir, pngs[0])
                    print(f"  Loading static sprite for {name} from character folder {static_path}")
                    animations = SpriteLoader._load_static_sprite(static_path, statuses)
                else:
                    print(f"  No PNG in character folder for {name}, using defaults")
    ...
    @staticmethod
    def _strip_flat_white_backdrop(surface):
        """If all four corners are opaque white, use white as colorkey (non-alpha PNG exports)."""
        ...
        def corner_is_opaque_white(c):
            if len(c) < 3:
                return False
            a = c[3] if len(c) > 3 else 255
            return a > 200 and c[0] > 245 and c[1] > 245 and c[2] > 245

        if all(corner_is_opaque_white(c) for c in corners):
            out = surface.copy()
    ...
    def _load_static_sprite(image_path, statuses):
        ...
        try:
            ...
            static_image = SpriteLoader._strip_flat_white_backdrop(static_image)
    ...
    @staticmethod
    def get_sprite_info(name, base_path, is_character=True):
        ...
        if os.path.exists(character_dir) and os.path.isdir(character_dir):
            ...
            for item in os.listdir(character_dir):
                ...
                if os.path.isdir(item_path):
                    ...
                    subdirs.append(f"{item} ({png_count} frames)")
            root_pngs = [f for f in os.listdir(character_dir) if f.lower().endswith('.png')]
            if subdirs:
                info['type'] = 'animated'
                info['directories'] = subdirs
            elif root_pngs:
                info['type'] = 'static'
                info['files'] = sorted(root_pngs)
            else:
                info['type'] = 'animated'
                info['directories'] = []
