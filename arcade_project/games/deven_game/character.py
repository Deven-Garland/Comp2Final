...
class Character(pygame.sprite.Sprite):
    ...
    def import_player_assets(self, animate=True):
        ...
        sprite_info = SpriteLoader.get_sprite_info(self.character_name, graphics_path("characters"))
    ...
    def clamp_to_world_bounds(self, bounds):
        """Keep the hitbox inside the map so gaps in FloorBlocks cannot leave the world."""
        if bounds is None:
            return
        if self.hitbox.left < bounds.left:
            self.hitbox.left = bounds.left
        if self.hitbox.right > bounds.right:
            self.hitbox.right = bounds.right
        if self.hitbox.top < bounds.top:
            self.hitbox.top = bounds.top
        if self.hitbox.bottom > bounds.bottom:
            self.hitbox.bottom = bounds.bottom
        self.rect.center = self.hitbox.center
    ...
    def get_preview_image():
        return graphics_path("test", "rock.png")